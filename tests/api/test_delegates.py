...
def test_all_calls_correct_url_with_default_params():
    ...
    assert responses.calls[0].request.url == 'http://127.0.0.1:4103/delegates?limit=100'
...
def test_all_calls_correct_url_with_passed_in_params():
    ...
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url.startswith('http://127.0.0.1:4103/delegates?')
...
def test_get_calls_correct_url():
    ...
    responses.add(
        responses.GET,
        'http://127.0.0.1:4103/delegates/{}'.format(delegate_id),
        json={'success': True},
        status=200
    )
    ...
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url == 'http://127.0.0.1:4103/delegates/12345'


def test_search_calls_correct_url_with_default_params():
    responses.add(
        responses.POST,
        'http://127.0.0.1:4103/delegates/search',
        json={'success': True},
        status=200
    )
    ...
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url == 'http://127.0.0.1:4103/delegates/search?limit=100'
...
def test_search_calls_correct_url_with_passed_in_params():
    ...
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url.startswith('http://127.0.0.1:4103/delegates/search?')
    assert 'page=5' in responses.calls[0].request.url
    assert 'limit=69' in responses.calls[0].request.url
    assert json.loads(responses.calls[0].request.body.decode()) == {'username': 'deadlock'}
...
def test_blocks_calls_correct_url_with_default_params():
    ...
    responses.add(
        responses.GET,
        'http://127.0.0.1:4103/delegates/{}/blocks'.format(delegate_id),
        json={'success': True},
        status=200
    )
    ...
    assert responses.calls[0].request.url == (
        'http://127.0.0.1:4103/delegates/12345/blocks?limit=100'
    )
...
def test_blocks_calls_correct_url_with_passed_in_params():
    ...
    assert responses.calls[0].request.url.startswith(
        'http://127.0.0.1:4103/delegates/12345/blocks?'
    )
    assert 'page=5' in responses.calls[0].request.url
    assert 'limit=69' in responses.calls[0].request.url
...
def test_voters_calls_correct_url_with_default_params():
    ...
    responses.add(
        responses.GET,
        'http://127.0.0.1:4103/delegates/{}/voters'.format(delegate_id),
        json={'success': True},
        status=200
    )
    ...
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url == (
        'http://127.0.0.1:4103/delegates/12345/voters?limit=100'
    )
...
def test_bvoters_calls_correct_url_with_passed_in_params():
    ...
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url.startswith(
        'http://127.0.0.1:4103/delegates/12345/voters?'
    )
    assert 'page=5' in responses.calls[0].request.url
    assert 'limit=69' in responses.calls[0].request.url
...
def test_voter_balances_calls_correct_url_with_default_params():
    ...
    responses.add(
        responses.GET,
        'http://127.0.0.1:4103/delegates/{}/voters/balances'.format(delegate_id),
        json={'success': True},
        status=200
    )
    ...
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url == 'http://127.0.0.1:4103/delegates/12345/voters/balances'
